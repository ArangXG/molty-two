...
class MoltyClient:
    ...
    async def list_rooms(self) -> list:
        data = await self._req("GET", "/rooms")
        if data is None:
            return []

        # Unwrap if wrapped in a dict key
        if isinstance(data, dict):
            raw_list = data.get("rooms") or data.get("data") or data.get("result") or []
            # If the dict itself has an 'id' key, it's a single room — wrap it
            if not raw_list and "id" in data:
                raw_list = [data]
        elif isinstance(data, list):
            raw_list = data
        else:
            log.warning(f"[API] list_rooms: unexpected type {type(data)}: {data}")
            return []

        # Normalize: API may return list of strings (room IDs) or list of dicts
        rooms = []
        for item in raw_list:
            if isinstance(item, str):
                # It's just a room ID string — fetch details or build minimal dict
                detail = await self._req("GET", f"/rooms/{item}")
                if detail and isinstance(detail, dict):
                    detail.setdefault("id", item)
                    rooms.append(detail)
                else:
                    # Fallback: minimal dict so RoomSelector can handle it
                    rooms.append({
                        "id":             item,
                        "current_players": 0,
                        "max_players":    99,
                ...
                # Normalise common field name variants
                item.setdefault("id",             item.get("room_id") or item.get("roomId") or "")
                item.setdefault("current_players", item.get("players") or item.get("playerCount") or 0)
                item.setdefault("max_players",     item.get("maxPlayers") or item.get("capacity") or 99)
                item.setdefault("type",            item.get("roomType") or "free")
                item.setdefault("entry_cost",      item.get("cost") or item.get("fee") or 0)
                rooms.append(item)
            else:
                log.debug(f"[ROOM] Skipping unknown room item type: {type(item)}")

        log.debug(f"[ROOM] Found {len(rooms)} rooms from API")
        return rooms
