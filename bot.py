...
class RoomSelector:
    ...
    def select(self, rooms: list, balance: float) -> Optional[dict]:
        ...
        for room in rooms:
            ...
            max_p   = room.get("max_players", 1)
            rtype   = room.get("type", "free").lower()
            cost    = float(room.get("entry_cost", 0))

            if current >= max_p:
                log.debug(f"[ROOM] Skip '{room.get('id')}' — FULL ({current}/{max_p})")
                continue

            if rtype == "paid" and balance < cost:
                log.debug(f"[ROOM] Skip '{room.get('id')}' — PAID ${cost}, balance=${balance:.2f}")
                continue
        ...
        best = max(available, key=lambda r: r.get("current_players", 0))
        log.info(
...
class MoltyBot:
    ...
    async def start(self):
        ...
        try:
            ...
            log.critical(f"[BOT] Fatal: {e}", exc_info=True)
        finally:
    ...
    async def _phase_room(self):
        ...
        if result:
            self.room_id  = rid
            self.match_id = result.get("match_id") or result.get("id") or rid
            self.stat_matches += 1
